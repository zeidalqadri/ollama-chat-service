...
<style>
    ...
    * { font-family: var(--font) !important; box-sizing: border-box; }
    ...
    /* Hide login elements when in chat-page mode */
    .chat-page [data-testid="stForm"],
    .chat-page [data-testid="stTabs"] {
...
def get_page_js(is_authenticated: bool) -> str:
    ...
    # When authenticated, aggressively remove any lingering login elements
    cleanup_js = '''
    // Remove ghost login elements immediately and on interval
    function cleanupLoginGhosts() {
        document.querySelectorAll('[data-testid="stForm"]').forEach(el => el.remove());
        document.querySelectorAll('[data-baseweb="tab-list"]').forEach(el => {
            if (el.textContent.includes('LOGIN')) {
                const tabs = el.closest('[data-testid="stTabs"]');
                if (tabs) tabs.remove();
            }
        });
        // Remove the BÖRAK header if found outside sidebar
        document.querySelectorAll('h1').forEach(el => {
            if (el.textContent.includes('BÖRAK') && !el.closest('[data-testid="stSidebar"]')) {
                el.closest('div')?.remove();
            }
        });
    }
    cleanupLoginGhosts();
    // Keep cleaning for 5 seconds to catch Streamlit re-renders
    const cleanupInterval = setInterval(cleanupLoginGhosts, 500);
...
if not st.session_state.authenticated:
    ...
    st.stop()  # CRITICAL: Stop here to prevent any chat UI from rendering when not authenticated
